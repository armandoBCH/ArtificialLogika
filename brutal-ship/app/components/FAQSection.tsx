            ...
            <div className="relative z-10 container mx-auto px-4 py-20 max-w-5xl">
                    ...
                    {faqs.map((item, index) => {
                        ...
                        return (
                            <motion.div
                                ...
                                key={item.id}
                                className="group bg-white dark:bg-neutral-800 border-2 border-black dark:border-white/25 rounded-lg shadow-elev-2 transition-all"
                            >
                                {/*
                                  Antes esto era un <motion.article onClick> sin
                                  tabIndex ni handler de teclado: el acordeón entero
                                  no se podía abrir sin mouse (WCAG 2.1.1, nivel A).
                                  Un <button> nativo dentro del heading resuelve
                                  foco, Enter/Espacio y semántica sin JS extra.
                                */}
                                <h3>
                                    <button
                                        type="button"
                                        ...
                                        className="w-full text-left p-6 flex justify-between items-center gap-4 cursor-pointer rounded-lg"
                                    >
                                        <span
                                            className={`text-xl font-bold transition-colors ${isOpen
                                        ...
                                        <span
                                            aria-hidden="true"
                                            className={`material-icons text-3xl font-bold rounded-full p-1 border-2 shrink-0 transition-all duration-300 ease-in-out transform ${isOpen
                                                ? "rotate-45 bg-transparent border-transparent text-neutral-500 dark:text-white"
                                                : "rotate-0 border-black dark:border-white/40 bg-neutral-100 dark:bg-neutral-700 text-neutral-900 dark:text-white"
                                                }`}
                                        >
                                            add
                                        </span>
                                ...
                                {/*
                                  El panel cerrado se oculta de verdad con `hidden`.
                                  Antes usaba grid-rows-[0fr] + opacity-0: quedaba
                                  invisible pero seguía en el árbol de accesibilidad,
                                  así que el lector de pantalla leía todas las
                                  respuestas siempre, abiertas o cerradas.
                                */}
                                <div
                                    id={panelId}
                                    role="region"
                ...
                <div className="mt-20 max-w-3xl mx-auto">
                    <div className="bg-white dark:bg-neutral-800 border-2 border-black p-8 md:p-12 rounded-lg shadow-neobrutalism relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
                        ...
                        <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackWhatsAppClick('faq')} className="relative z-10 bg-whatsapp hover:bg-[#20BD5A] text-white font-bold text-lg px-8 py-4 rounded border-2 border-black shadow-neobrutalism active:shadow-neobrutalism-hover active:translate-x-[2px] active:translate-y-[2px] transition-all flex items-center gap-2 group whitespace-nowrap">
